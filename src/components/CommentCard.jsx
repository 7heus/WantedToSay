...
export default function CommentCard({ comment }) {
  ...
  useEffect(() => {
    setLiked(comment.reactions.includes(user._id));
  }, [user, comment.reactions]);

  useEffect(() => {
    setLikeCount(comment.reactions.length);
  }, [comment.reactions]);
  ...
  return (
    <>
      {!isRemoved && (
        <div className="comment-card">
          <div className="comment-header">
            <p className="user-name">{comment.userName || "Anonymous"}</p>
          </div>
          <div className="comment-body">
            {isEditing ? (
              <textarea
